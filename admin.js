// admin.js — FINAL 100% WORKING, REAL-TIME, MOBILE-PERFECT EMPIRE DASHBOARD
...
router.addLog = (type, phone, msg) => {
  logs.unshift({
    ...
    type,
    phone,
    msg: String(msg).slice(0, 80) + (msg.length > 80 ? "..." : "")
  });
...
<body class="bg-gray-100 min-h-screen" x-data="{page:'dashboard',sidebar:false,selected:[],msg:'',b1:'',b2:''}">
  <button @click="sidebar=!sidebar" class="lg:hidden fixed top-4 left-4 z-50 bg-white p-3 rounded-xl shadow">
    <i class="fas fa-bars text-xl"></i>
  </button>

  <aside :class="sidebar?'translate-x-0':'-translate-x-full'" class="fixed inset-y-0 left-0 z-40 w-64 bg-white shadow-2xl transform transition lg:translate-x-0">
    <div class="p-6 border-b"><h1 class="text-2xl font-black text-blue-600">BlackLab</h1></div>
    <nav class="p-4 space-y-2">
      <button @click="page='dashboard';sidebar=false" :class="page==='dashboard'?'bg-blue-50 text-blue-700':''" class="w-full text-left px-4 py-3 rounded-lg flex items-center gap-3"><i class="fas fa-home"></i> Dashboard</button>
      <button @click="page='customers';sidebar=false" :class="page==='customers'?'bg-blue-50 text-blue-700':''" class="w-full text-left px-4 py-3 rounded-lg flex items-center gap-3"><i class="fas fa-users"></i> Customers (${users.length})</button>
      <button @click="page='packages';sidebar=false" :class="page==='packages'?'bg-blue-50 text-blue-700':''" class="w-full text-left px-4 py-3 rounded-lg flex items-center gap-3"><i class="fas fa-box"></i> Packages</button>
      <button @click="page='broadcast';sidebar=false" :class="page==='broadcast'?'bg-blue-50 text-blue-700':''" class="w-full text-left px-4 py-3 rounded-lg flex items-center gap-3"><i class="fas fa-paper-plane"></i> Broadcast</button>
      <button @click="page='logs';sidebar=false" :class="page==='logs'?'bg-blue-50 text-blue-700':''" class="w-full text-left px-4 py-3 rounded-lg flex items-center gap-3"><i class="fas fa-history"></i> Logs</button>
    </nav>
  </aside>
  ...
  <main class="lg:ml-64 p-6 pt-20 lg:pt-6">
    ...
    <div x-show="page==='customers'">
      ...
      <div class="bg-white rounded-2xl shadow overflow-hidden">
        <table class="w-full text-sm">
          ...
          <tbody>
            \( {users.map(p => `<tr><td class="px-6 py-4 font-medium"> \){p}</td><td class="text-center"><input type="checkbox" value="${p}" x-model="selected"></td></tr>`).join("")}
          </tbody>
        </table>
      </div>
    ...
    <div x-show="page==='packages'">
      <div class="flex justify-between mb-6"><h2 class="text-3xl font-bold">Packages</h2>
        <button @click="$refs.modal.showModal()" class="bg-blue-600 text-white px-6 py-3 rounded-xl">+ Add</button>
      </div>
    ...
    <div x-show="page==='broadcast'" class="max-w-2xl mx-auto">
      ...
      <div class="bg-white rounded-2xl shadow p-8 space-y-6">
        <textarea x-model="msg" rows="6" placeholder="Message..." class="w-full border rounded-xl p-4"></textarea>
        ...
        <button @click="send()" class="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-4 rounded-xl" 
  ...
  <dialog x-ref="modal" class="rounded-2xl p-8 w-full max-w-md bg-white">
  ...
  <script>
    function send() {
      if (!this.msg) return alert("Write message");
      const buttons = [];
      ...
      fetch('/api/broadcast', {
        ...
        alert("Sent to " + this.selected.length);
...
router.get("/api/stats", (req, res) => res.send(`
  <div class="bg-white rounded-2xl shadow p-6"><p class="text-gray-500 text-sm">Received</p><p class="text-4xl font-bold text-green-600">${stats.received}</p></div>
  <div class="bg-white rounded-2xl shadow p-6"><p class="text-gray-500 text-sm">Sent</p><p class="text-4xl font-bold text-blue-600">${stats.sent}</p></div>
  <div class="bg-white rounded-2xl shadow p-6"><p class="text-gray-500 text-sm">Customers</p><p class="text-4xl font-bold text-purple-600">${storage.getUsers().length}</p></div>
  <div class="bg-white rounded-2xl shadow p-6"><p class="text-gray-500 text-sm">Packages</p><p class="text-4xl font-bold text-orange-600">${storage.getPackages().length}</p></div>
`));
...
router.get("/api/logs", (req, res) => res.send(`
  <table class="w-full text-sm">
    ...
    <tbody>
      \( {logs.length ? logs.map(l => `<tr class=" \){l.type==='IN'?'bg-green-50':'bg-blue-50'}">
        ...
        <td>${l.msg}</td>
      </tr>`).join("") : "<tr><td colspan='4' class='text-center py-8 text-gray-500'>No logs yet</td></tr>"}
    </tbody>
  </table>
`));
...
router.get("/api/packages", (req, res) => {
  ...
  const grouped = pkgs.reduce((a,c)=>(a[c.category]=(a[c.category]||[])).push(c)&&a,{});
  let html = "";
  for (const [cat, items] of Object.entries(grouped)) {
    html += `<h3 class="text-xl font-bold capitalize mb-4">${cat}</h3><div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">`;
    items.forEach(p => html += `<div class="bg-white rounded-2xl shadow p-6 border">
