...
// Webhook – incoming messages & button replies
  ...
  try {
    ...
    if (message?.interactive?.button_reply?.id) {
      ...
      if (buttonId === "back_main") {
        ...
        return sendMenu(from, "main");
      }

      if (buttonId.startsWith("bundle_")) {
        const bundle = buttonId.replace("bundle_", "").replace("_", " ").toUpperCase();
        await axios.post(
          `https://graph.facebook.com/v20.0/${PHONE_NUMBER_ID}/messages`,
          {
            messaging_product: "whatsapp",
    ...
    // Any other message (text, image, etc.) → show main menu
    sessions.delete(from);
...
// Home route
app.get("/", (req, res) => {
  res.send("BlackLab WhatsApp Bot is LIVE – built with ❤️ by blacklab tech");
});
